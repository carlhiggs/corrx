...
sliderInput("rho2", "Corr(x,y) in Dz twins:",min=0, max=1, value=0.2, step=0.1)
...
renderPlot({
    ...
    #   fit <- rbind(fit,
    #                cbind("test" = x,
    #                      "n"    = min(obs()[,"n"]):max(obs()[,"n"]),
    #                      "rho1" = splinefun(obs()%>%filter(test==x)%>%select("log_n"),
    #                                         obs()%>%filter(test==x)%>%select("rho1"),
    #                                         method = "monoH.FC")(log2(min(obs()$n):max(obs()$n)))))
    # }
    # fit$n <- as.integer(fit$n)
    # fit$power <- as.double(fit$rho1)
